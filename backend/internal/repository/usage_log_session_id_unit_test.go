...
func TestPrepareUsageLogInsert_RequestedReasoningEffortArgWiring(t *testing.T) {
	...
	require.Len(t, prepared.args, len(usageLogInsertArgTypes))
	require.Equal(t, "text", usageLogInsertArgTypes[56], "requested_reasoning_effort must follow reasoning_effort")
	require.Equal(t, "text", usageLogInsertArgTypes[55], "reasoning_effort arg type must stay text")

	forwardedArg, ok := prepared.args[55].(sql.NullString)
	require.True(t, ok)
	require.True(t, forwardedArg.Valid)
	require.Equal(t, forwarded, forwardedArg.String)

	requestedArg, ok := prepared.args[56].(sql.NullString)
	require.True(t, ok)
