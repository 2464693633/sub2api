export default {
    settings: {
      ...
      openaiExperimentalScheduler: {
        ...
        sessionStickyTitle: '会话粘性',
        sessionStickyDescription: '开启后同一会话固定使用首次选中的账号（含母账号粘连）。关闭后每轮请求都重新按优先级/负载选号。',
        previousStickyTitle: '上一条响应粘性',
        previousStickyDescription: '开启后携带 previous_response_id 的请求优先路由到产生该响应的账号。关闭后可重建上下文的请求会剥离该参数重建，换号不再受限于原账号。',
        strictPriorityTitle: '严格优先级',
        strictPriorityDescription: '开启后跳过打分与加权随机，严格按「优先级 → 负载率 → 排队数 → 最久未用」排序逐个尝试：高优先级账号可用时流量不会落到低优先级账号。',
        breakerTitle: '账号健康熔断',
        breakerDescription: '窗口内连续失败达到阈值时临时禁用该账号，流量切换到其他账号；冷却到期后自动恢复。',
        breakerThreshold: '失败阈值（次）',
        breakerWindow: '统计窗口（分钟）',
        breakerCooldown: '禁用时长（分钟）',
        breakerScope: '适用范围',
        breakerScopePool: '仅池模式 API Key 账号',
        breakerScopeAll: 'OpenAI 平台全部账号',
        breakerTimeoutsTitle: '超时计入失败',
        breakerTimeoutsDescription: '上游无响应超时（客户端取消除外）计入失败窗口。',
        breakerStreamErrorsTitle: '流内失败计入',
        breakerStreamErrorsDescription: '响应建立后的失败（如首 token 前报错、连接中断）计入失败窗口。',
