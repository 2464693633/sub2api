export default {
    settings: {
      ...
      openaiExperimentalScheduler: {
        ...
        sessionStickyTitle: 'Session stickiness',
        sessionStickyDescription: 'When enabled, a session keeps using the account chosen on its first request (including guardian-parent pinning). When disabled, every request is re-scheduled by priority/load.',
        previousStickyTitle: 'Previous-response stickiness',
        previousStickyDescription: 'When enabled, requests carrying previous_response_id prefer the account that produced it. When disabled, requests whose context can be rebuilt strip the parameter and may be routed to any account.',
        strictPriorityTitle: 'Strict priority',
        strictPriorityDescription: 'When enabled, scoring and weighted randomization are skipped; candidates are tried in deterministic order (priority → load rate → queue → LRU), so traffic never falls to lower-priority accounts while higher ones are usable.',
        breakerTitle: 'Account health breaker',
        breakerDescription: 'Temporarily disables an account once consecutive failures reach the threshold within the window; traffic moves to other accounts and the account recovers automatically after the cooldown.',
        breakerThreshold: 'Failure threshold (count)',
        breakerWindow: 'Window (minutes)',
        breakerCooldown: 'Cooldown (minutes)',
        breakerScope: 'Scope',
        breakerScopePool: 'Pool-mode API Key accounts only',
        breakerScopeAll: 'All OpenAI platform accounts',
        breakerTimeoutsTitle: 'Count timeouts as failures',
        breakerTimeoutsDescription: 'Upstream no-response timeouts (excluding client cancellations) count toward the failure window.',
        breakerStreamErrorsTitle: 'Count in-stream failures',
        breakerStreamErrorsDescription: 'Failures after the response is established (e.g. errors before the first token, connection drops) count toward the failure window.',
