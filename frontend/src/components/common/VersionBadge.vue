...
const repository = computed(() => appStore.updateRepository)
const releaseUrl = computed(() => {
  const apiReleaseUrl = releaseInfo.value?.html_url?.trim()
  if (apiReleaseUrl && apiReleaseUrl !== '#') return apiReleaseUrl

  return getGitHubReleaseUrl(
    repository.value,
...
const scriptRollbackCommand = computed(() => {
  ...
  const scriptUrl = getInstallScriptUrl(repository.value, tag)
  if (!scriptUrl) return ''
  return `curl -sSL ${scriptUrl} | sudo bash -s -- rollback ${tag}`
})

const dockerRollbackCommand = computed(() => {
  if (!selectedRollbackVersion.value) return ''
  const image = getGhcrImage(repository.value)
  if (!image) return ''
  return [
